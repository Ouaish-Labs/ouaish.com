...
Report: "Worktree ready at `~/Developer/<branch-name>`. Starting implementation."
...
Immediately after PR creation, post a compliance comment with **live data pasted from commands**. No yes/no self-reports — every field requires output that can only come from running the command.

**Before writing the comment, run these commands and paste their output:**

```bash
# 1. Verify architect ran — paste the agent ID from Phase 0
#    (if you can't find it, Phase 0 was skipped)

# 2. Get CI run ID
gh run list --branch <branch-name> --limit 1 --json databaseId -q '.[0].databaseId'

# 3. Check for external reviewer comments (first check — may be empty, that's OK)
gh api repos/{owner}/{repo}/pulls/{number}/comments --jq 'length'
```

Then post the compliance comment using the actual values:
...
| Phase | Evidence |
|-------|----------|
| Phase 0 (architect) | agent: `<paste agent ID>` / ❌ SKIPPED: _reason_ |
| Phase 0½ (scope gate) | `<N>` issues created, `<N>` deferred / ⏭️ n/a (no epic) |
| Plan reconciliation | `<N>`/`<N>` verify items — see pre-commit checklist |
| Phase B¾ (completeness) | `<N>`/`<N>` epic issues resolved / ⏭️ n/a (no epic) |
| External comments | `<paste count>` at `<timestamp>` — will recheck in Final Summary |
| CI status | run: `<paste run ID>` |

_Generated by /worktree-pr_
EOF
)"
```

**Key design:** The external comment count is timestamped. It may be 0 at PR creation time — that's expected. The Final Summary (Step 5) does a mandatory recheck after the review loop, when enough time has passed for bots to comment.

**If any field shows ❌ SKIPPED, the user sees it permanently on the PR.**
